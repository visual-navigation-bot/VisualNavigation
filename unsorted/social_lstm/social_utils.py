...
class SocialDataLoader():
    ...
    def __init__(self, batch_size=50, seq_length=5, maxNumPeds=40, datasets=[0, 1, 2, 3, 4], forcePreProcess=False):
        ...
        # choose dataset to be used, indexing over list self.data_dirs
    ...
    def frame_preprocess(self, data_dirs, data_file):
        ...
        # all_frame_data[which_dataset][which_frame, which_ped, 3_dim_data]
        all_frame_data = []
        ...
        for directory in data_dirs:
            ...
            numPeds_data.append([]) # use [] to seperate different datasets
            # Initialize the numpy array for the current dataset
            # each nparray (corresponding to a dataset)in all_frame_data is 
            # of shape (which_frame, which_ped, 3_dim_data_for_a_ped)
            all_frame_data.append(np.zeros((numFrames, self.maxNumPeds, 3)))

            # index to maintain the current frame
            curr_frame = 0
            # for each frame_ID in current dataset
            for frame in frameList: # frame=frame_ID
                # Extract all pedestrians in current frame
                # pedsInframe shape = 4 x num_data_with_the_same_frameID_as_"frame"
                pedsInFrame = data[:, data[0, :] == frame]

                # Extract peds list
                # pedsList is a list of ped_ID in current frame_ID
                ...
                for ped in pedsList:
                    ...
                    # pedsInFrame[1,:] is all ped_ID in the current frame
                    # the last [0] is to make a list to a scalar
    ...
    def load_preprocessed(self, data_file):
        ...
        # counter is simply how many possible sequences in all dataset
        self.num_batches = int(counter/self.batch_size)
        # On an average, we need twice the number of batches to cover the data
        # due to randomization introduced, UNKNOWN, why 2
    ...
    def next_batch(self, randomUpdate=True):
        ...
        RETURNS:
            x_batch: a list of nparrays, [batch_idx(batch_size)][which_frame(seq_length),which_ped(maxNumPed),3]
            y_batch: a list of nparrays, [batch_idx(batch_size)][which_frame(seq_length),which_ped(maxNumPed),3]
        '''
        # Source data
        x_batch = []
        # Target data
        ...
        while i < self.batch_size:
            # Extract the (all) frame data of the current dataset
            ...
            if idx + self.seq_length < frame_data.shape[0]:
                ...
                seq_frame_data = frame_data[idx:idx+self.seq_length+1, :] # shape=(seq_length+1,maxNumPeds,3)
                seq_source_frame_data = frame_data[idx:idx+self.seq_length, :] # shape=(seq_length,maxNumPeds,3)
                seq_target_frame_data = frame_data[idx+1:idx+self.seq_length+1, :] # shape=(seq_length,maxNumPeds,3)
                # Number of unique peds in this sequence of frames
                # acutally pedID_list is not a list but a nparray with shape (num_of_ped,1?)
                ...
                # for each sequence index (seq also alias to frame)
                for seq in range(self.seq_length):
                    # current frame(seq) data --> shape=(maxNumPed,3)
                    ...
                    for ped in range(numUniquePeds):
                        ...
                        # remember all_frame_data is initialized to np.zeros((numFrames,maxNumPed,3))
                        # therefore if pedID is 0, it means that num_peds doesn't exceed maxNumPed, and
                        # there is no data in pedID==0
                        if pedID == 0: 
                            continue
                        else:
                            # source data of pedID'th ped in seq'th frame
                            sped = np.squeeze(sseq_frame_data[sseq_frame_data[:, 0] == pedID, :])
                            # target data of pedID'th ped in seq'th frame
                            # --> simply source data of pedID'th ped in (seq+1)'th frame
