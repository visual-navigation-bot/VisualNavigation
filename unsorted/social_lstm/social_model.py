...
class SocialModel():
    ...
    def __init__(self, args, infer=False):
        ...
        # stored states for successive inference
        self.stored_states = np.zeros((self.maxNumPeds,self.rnn_size*2))
            ...
            for ped in range(args.maxNumPeds):
                ...
                if not infer:
                    ...
                    with tf.name_scope("increment_cost"):
                        ...
                        self.cost = tf.select(tf.logical_or(tf.equal(pedID, nonexistent_ped),
                                           tf.equal(target_pedID, nonexistent_ped)), self.cost, tf.add(self.cost, lossfunc))
                        self.counter = tf.select(tf.logical_or(tf.equal(pedID, nonexistent_ped), 
                                           tf.equal(target_pedID, nonexistent_ped)), self.counter, tf.add(self.counter, self.increment))
        if not infer:
    ...
    def sample_init(self, sess, traj, grid):
        '''
        FUNC: initialize dynamics of pedestrians given their observed trajectory
        Argument:
            sess: tf session to be run on
            traj: observed trajectory, used to encode pedestrians dynamics into LSTM states
            grid: grid data corresponding to traj
        '''
        # for now, states is initial zero state
        states = sess.run(self.LSTM_states)
        # for each frame in observed sequence (traj)
        for idx, frame in enumerate(traj[:-1]):
            # reshape frame shape from (MNP,3) to (1,MNP,3)
            data = np.reshape(frame, (1,self.maxNumPeds,3))
            # target_data is the next time step of frame. Also, reshape from (MNP,3) tp (1,MNP,3)
            target_data = np.reshape(traj[idx+1], (1,self.maxNumPeds,3))#TODO
            # used for social tensor, reshape from (MNP,grid_size**2) to (1,MNP,grid_size**2)
            grid_data = np.reshape(grid[idx,:], (1,self.maxNumPeds,self.maxNumPeds,self.grid_size*self.grid_size))

            # define data to be fed and to be fetched
            feed = {
            ...
            fetch = self.final_states
            # run session
            states = sess.run(fetch, feed)
    ...
    def sample_one_step(self, sess, prev_data, prev_grid_data, dimensions):
        ...
        for pedindex, pedoutput in enumerate(output):
            ...
            '''
            # store to matrix
            next_data[0, pedindex, :] = [prev_data[0, pedindex, 0], next_x, next_y]
        # obtain grid data of predicted (next) step
        next_grid_data = getSequenceGridMask(next_data, dimensions, self.args.neighborhood_size, self.grid_size)
